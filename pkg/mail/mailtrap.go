...
func NewMailtrapProvider(config *MailConfig) (EmailProviderService, error) {
	...
	if !ok {
		return nil, utils.NewError("mailtrap config not found", utils.ErrCodeInternal)
	}
...
func (m *MailtrapProvider) SendMail(ctx context.Context, email *Email) error {
	...
	if response.StatusCode != http.StatusOK {
		...
		return utils.NewError(fmt.Sprintf("Unexpected response status code %d: %s", response.StatusCode, string(body)), utils.ErrCodeBadGateway)
	}
